...
http_method_t request_parse_method(const char *method_str) {
    ...
    if (strcasecmp(method_str, "PUT") == 0) return METHOD_PUT;
    if (strcasecmp(method_str, "DELETE") == 0) return METHOD_DELETE;
    if (strcasecmp(method_str, "HEAD") == 0) return METHOD_HEAD;
    if (strcasecmp(method_str, "OPTIONS") == 0) return METHOD_OPTIONS;
